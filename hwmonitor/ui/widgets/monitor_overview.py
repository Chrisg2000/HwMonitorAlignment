...
class MonitorOverview(QWidget):
    ...
    def __init__(self, vscreen: VScreen, parent=None):
        ...
        self.vscreen.monitor_added.connect(self._monitor_added)
        self.vscreen.monitor_removed.connect(self._monitor_removed)
        self.vscreen.monitor_model_reset.connect(self._reset)
    ...
    def _reset(self, model_reset=True):
        self.vscreen_width, self.vscreen_height = self.vscreen.size
        self.x_offset, self.y_offset = self.vscreen.offset
    ...
    def draw_widget(self, painter: QPainter):
        ...
        monitor: Monitor
        for index, monitor in enumerate(self.vscreen.monitors, 1):
